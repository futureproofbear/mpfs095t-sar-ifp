...
set proj "$here/libero_disc"
file delete -force $proj

new_project \
    -location $proj -name {sar_accel} -project_description {SAR accelerator (fresh, TLAST fix)} \
    -hdl {VERILOG} -family {PolarFireSoC} -die {MPFS095T} -package {FCSG325} \
    -speed {-1} -die_voltage {1.0} -part_range {EXT} -ondemand_build_dh {1}
...
import_mss_component -file "$here/mss_discovery/out/MPFS_DISCOVERY_KIT_MSS.cxz"
build_design_hierarchy
