 ...
 */
/* HSS-payload config: HSS starts ONLY the owner hart (u54_1) at the payload
 * exec addr, so the app runs on hart 1 alone. E51 (hart0) and U54_2..4 are not
 * started here -> FIRST_HART = LAST_HART = 1 (the "loaded by a bootloader that
 * kicks off U54_1" use-case in the stock comment above). */
 ...
 *
 */
/* HSS is the previous boot stage: it trains DDR, sets clocks/IO and loads this
 * payload to DDR. So the payload must NOT redo HW config (no MPFS_HAL_HW_CONFIG)
 * -- re-training a live DDR would hang. IMAGE_LOADED_BY_BOOTLOADER = 1. */
